...
system_state_t *init(const char *bfn, const char *gfn) {
  ...
  cdrom::init(system->cdrom_state, gfn);
  cpu::init(system->cpu_state);
  bus::init(system, bfn);

  input::init(system->input_state);
...
int main(int argc, char *argv[]) {
  ...
  system_state_t *system = init(argv[1], argv[2]);
