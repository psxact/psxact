...
// --========--
  ...
  uint32_t cpu_core::get_register(uint32_t index) {
  ...
  uint32_t cpu_core::get_register_forwarded(uint32_t index) {
    return regs.gp[index];
  }

  void cpu_core::set_rd(uint32_t value) {
    regs.gp[decode_rd()] = value;
    regs.gp[0] = 0;
  }

  void cpu_core::set_rt(uint32_t value) {
    regs.gp[decode_rt()] = value;
    regs.gp[0] = 0;
  }

  void cpu_core::set_rt_load(uint32_t value) {
  ...
  uint32_t cpu_core::get_rt() {
    return get_register(decode_rt());
  }

  uint32_t cpu_core::get_rt_forwarded() {
    return get_register_forwarded(decode_rt());
  }

  uint32_t cpu_core::get_rs() {
...
// --============--
  ...
  void cpu_core::op_addiu() {
    set_rt(get_rs() + decode_iconst());
  }

  void cpu_core::op_addu() {
    set_rd(get_rs() + get_rt());
  }

  void cpu_core::op_and() {
    set_rd(get_rs() & get_rt());
  }

  void cpu_core::op_andi() {
    set_rt(get_rs() & decode_uconst());
  }

  void cpu_core::op_beq() {
    if (get_rs() == get_rt()) {
  ...
  void cpu_core::op_break() {
    enter_exception(0x09);
  }

  void cpu_core::op_bxx() {
    // bgez rs,$nnnn
    // bgezal rs,$nnnn
    // bltz rs,$nnnn
    // bltzal rs,$nnnn
  ...
  void cpu_core::op_cop1() {
    enter_exception(0xb);
  }

  void cpu_core::op_cop2() {
    if (code & (1 << 25)) {
  ...
  void cpu_core::op_cop3() {
    enter_exception(0xb);
  }

  void cpu_core::op_div() {
  ...
  void cpu_core::op_jal() {
    regs.gp[31] = regs.next_pc;
    regs.next_pc = (regs.pc & 0xf0000000) | ((code << 2) & 0x0ffffffc);
    is_branch = true;
  }

  void cpu_core::op_jalr() {
  ...
  void cpu_core::op_lwc0() {
    printf("cpu_core::op_lwc0(0x%08x)\n", code);
  }

  void cpu_core::op_lwc1() {
    printf("cpu_core::op_lwc1(0x%08x)\n", code);
  }

  void cpu_core::op_lwc2() {
    uint32_t address = get_rs() + decode_iconst();
    if (address & 3) {
      enter_exception(0x4);
    }
  ...
  void cpu_core::op_lwc3() {
    printf("cpu_core::op_lwc3(0x%08x)\n", code);
  }

  void cpu_core::op_lwl() {
    uint32_t address = get_rs() + decode_iconst();
  ...
  void cpu_core::op_mfhi() {
    set_rd(regs.hi);
  }

  void cpu_core::op_mflo() {
    set_rd(regs.lo);
  }

  void cpu_core::op_mthi() {
    regs.hi = get_rs();
  }

  void cpu_core::op_mtlo() {
    regs.lo = get_rs();
  }

  void cpu_core::op_mult() {
  ...
  void cpu_core::op_swc0() {
    printf("cpu_core::op_swc0(0x%08x)\n", code);
  }

  void cpu_core::op_swc1() {
    printf("cpu_core::op_swc1(0x%08x)\n", code);
  }

  void cpu_core::op_swc2() {
    uint32_t address = get_rs() + decode_iconst();
    if (address & 3) {
      enter_exception(0x5);
    }
  ...
  void cpu_core::op_swc3() {
    printf("cpu_core::op_swc3(0x%08x)\n", code);
  }

  void cpu_core::op_swl() {
    uint32_t address = get_rs() + decode_iconst();
    uint32_t data = read_data(BUS_WIDTH_WORD, address & ~3);
  ...
  void cpu_core::op_syscall() {
    enter_exception(0x08);
  }

  void cpu_core::op_xor() {
    set_rd(get_rs() ^ get_rt());
  }

  void cpu_core::op_xori() {
    set_rt(get_rs() ^ decode_uconst());
  }

  void cpu_core::op_und() {
    enter_exception(0xa);
  }
}
